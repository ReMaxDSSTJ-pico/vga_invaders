...
#   @param pico			"-p", output one single array for PICOVGA01 examples [optional]
...
#	parser.add_argument ("-d", "--double", help="define data in 'words' rather than bytes", action="store_true")
	parser.add_argument ("-p", "--pico", help="generate PICOVGA01 .cpp file for VGA Examples with only one array of 1 byte per pixel)", action="store_true")
	...
	if (pico):
		...
		print ('')
		print ('// for PICOVGA01 examples. Format 1 byte, single array')
	...
	for f in args.infile:
		if f == '-':
			sys.exit()
		bmp2hex(f.name, tablewidth, sizebytes, invert, raw, named, double, pico, xbm)
...
def bmp2hex(infile, tablewidth, sizebytes, invert, raw, named, double, pico, xbm):
	...
	if (raw):
		...
		print ('// format: 8-bit indexed pixel graphics, palette')
		print ('//  image width  :' + str(pixelWidth) + ' pixels')
		print ('//  image height :' + str(pixelHeight)+ ' lines')
		print ('')
		print ('// ' + pixelDataType)
		print ('// bits per pixel {2}'.format(pixelWidth, pixelHeight, bitDepth))
		print ('\n')
		print ('  const u8 ' + tablename + '[{0}]'.format(round(byteWidth * pixelHeight / dataByteLength)) + ' __attribute__ ((aligned(4))) = {')
